...
static void banner_load() {
	...
	bomb_explosion = image_load(BOMB_EXPLOSION);
}
...
static void map_unload() {
	...
	SDL_FreeSurface(bomb_explosion);
}
...
static void princess_load() {
	princess_img = image_load(PRINCESS);
}

static void princess_unload() {
	SDL_FreeSurface(princess_img);
}
...
static void bomb_load() {
	...
	bomb_img[0] = image_load(BOMB_EXPLOSION);
}
...
SDL_Surface* sprite_get_bomb(int var) {
	assert(bomb_img[var]);
	return bomb_img[var];
}
